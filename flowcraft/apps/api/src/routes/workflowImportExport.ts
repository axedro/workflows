...
export async function workflowImportExportRoutes(fastify: FastifyInstance) {
  // Export workflow
  fastify.post('/workflows/:id/export', {
    schema: {
      ...
      if (!data.overwrite) {
        ...
        if (existingWorkflow) {
          return reply.status(409).send({
            error: 'Conflict',
            message: 'Workflow with this name already exists. Use overwrite=true to replace it.',
          });
        }
      }
  ...
  // Export template
  fastify.post('/workflow-templates/:id/export', {
    schema: {
    ...
    try {
      ...
      const template = await templateService.getTemplateById(id, user.userId, user.organizationId);
      if (!template) {
        return reply.status(404).send({
          error: 'Not Found',
  ...
  // Import template
  fastify.post('/workflow-templates/import', {
    schema: {
    ...
    try {
      ...
      if (!data.overwrite) {
        ...
        if (existingTemplate) {
          return reply.status(409).send({
            error: 'Conflict',
            message: 'Template with this name already exists. Use overwrite=true to replace it.',
          });
        }
      }
  ...
  fastify.post('/workflows/bulk-export', {
    schema: {
    ...
    try {
      const user = request.user as any;
      const { workflowIds, includeVersions, includeExecutions } = z.object({
      ...
      for (const workflowId of workflowIds) {
        const workflow = await workflowService.getWorkflowById(workflowId, user.userId, user.organizationId);
        if (workflow) {
          const exportData: any = {
            id: workflow.id,
            name: workflow.name,
  ...
  fastify.post('/workflows/bulk-import', {
    schema: {
    ...
    try {
      const user = request.user as any;
      const { workflows, overwrite } = z.object({
      ...
      for (const workflowData of workflows) {
        try {
          // Validate workflow definition
          const validation = await validationService.validateWorkflow(workflowData.definition as any);
          if (!validation.isValid) {
            results.push({
