...
import { z } from 'zod';
import { WorkflowService, createWorkflowSchema, updateWorkflowSchema, createVersionSchema } from '../services/workflow.service.js';
...
export async function workflowRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/', {
    ...
    try {
      ...
      const validation = await validationService.validateWorkflow(data.definition as any);
      ...
      const workflow = await workflowService.createWorkflow(data, user.userId);
  ...
  fastify.get('/:id', {
    ...
    try {
      ...
      const workflow = await workflowService.getWorkflowById(id, user.userId, user.organizationId);
      if (!workflow) {
  ...
  fastify.put('/:id', {
    ...
    try {
      ...
      const workflow = await workflowService.updateWorkflow(id, data, user.userId);
      return reply.send(workflow);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return reply.status(400).send({
  ...
  fastify.delete('/:id', {
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (error.message === 'Cannot delete workflow with existing executions') {
  ...
  fastify.post('/:id/duplicate', {
    ...
    try {
      ...
      const workflow = await workflowService.duplicateWorkflow(id, name, user.userId);
      return reply.status(201).send(workflow);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return reply.status(400).send({
  ...
  fastify.post('/:id/versions', {
    ...
    try {
      ...
      const version = await workflowService.createVersion(id, data, user.userId);
  ...
  fastify.get('/:id/versions/:version', {
    ...
    try {
      const user = request.user as any;
      const { id, version } = workflowVersionParamsSchema.parse(request.params);

      const workflowVersion = await workflowService.getWorkflowVersion(id, version, user.userId);
      if (!workflowVersion) {
        return reply.status(404).send({
          error: 'Not Found',
  ...
  fastify.post('/:id/validate', {
    ...
    try {
      const user = request.user as any;
      const { id } = workflowIdParamsSchema.parse(request.params);
      const { definition } = z.object({ definition: z.record(z.any()) }).parse(request.body);
      ...
      const validation = await validationService.validateWorkflow(definition as any);
