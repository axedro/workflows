...
import { z } from 'zod';
import { WorkflowTemplateService, createTemplateSchema, updateTemplateSchema } from '../services/workflowTemplate.service.js';
...
export async function workflowTemplateRoutes(fastify: FastifyInstance) {
  ...
  fastify.put('/:id', {
    ...
    try {
      ...
      const template = await templateService.updateTemplate(id, data, user.userId);
      return reply.send(template);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return reply.status(400).send({
  ...
  fastify.post('/:id/duplicate', {
    ...
    try {
      ...
      const template = await templateService.duplicateTemplate(id, name, user.userId, user.organizationId);
      return reply.status(201).send(template);
    } catch (error) {
      if (error instanceof z.ZodError) {
        return reply.status(400).send({
